...
try:  # пробуем выполнить тело кода
    ...
    sendmessage(myvkid, f'Ошибка подключения к серверу по причине: {error}! Выключаемся...')
        ...
        if len(message.split(" ")) == 3:
            ...
            if message.split(" ")[1] == 'добавить':  # проверяем что ВТОРОЕ СЛОВО - 'добавить'
                ...
                if 'Player is not whitelisted' in com:
                    sendmessage(id,
                                f'Не удалось удалить {nick} из вайтлиста. Его там и не было!')
                else:
...
#    try:
 ...
 #       sendmessage(myvkid, 'Пробуем перезапуститься...')
  ...
  #      sendmessage(myvkid, 'Пробуем перезапуститься...')
  #      pass
  #  pass  # забиваем огромный болт на ошибку, и скипаем её
