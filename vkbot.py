...
def rconcommand(com,vid):
    if id in adminsid:
        try: command = rcon.command(com)
        except Exception as eror: sendmessage(vid, f'Не удалось выполнить команду по причине {eror}')
        else:
            sendmessage(vid,f'Выполнил команду {com}, '
            f'Вывод команды: {command}')
    else: sendmessage(vid,'Ты не админ!')
...
def banplayer(message, id):
    ...
    else: sendmessage(id, 'Ты не админ!')
        ...
        if event.to_me:
            ...
            if message.split(" ")[0] in ping: #если в сообщении которое отправил челикс ПЕРВОЕ слово это одно из массива ping
                try:
                    ...
                    sendmessage(id, f'Сервер не отвечает по причине {e}. Свяжитесь с админом как можно скорее.') #отправляем пользователю это сообщение чтобы скрипт не выключился
                else: #если нет ошибок
                    sendmessage(id, f'Пинг: {latency} мс, на сервере сейчас {status.players.online} игрока(ов)') #пишем это
                ...
                if id == myvkid and len(message.split(" ")) > 1: #проверяем является ли айди написавшего - вашим
                    if message.split(" ")[1] == 'добавить': #проверяем что ВТОРОЕ СЛОВО - добавить
                        com1 = rcon.command("whitelist add " + message.split(" ")[2]) #отправляем ркон команду на добавление вайтлиста с третьим словом в сообщении пользователя
                        ...
                        com1 = rcon.command("whitelist remove " + message.split(" ")[2]) #отправляем ркон команду на удаление из вайтлиста с третьим словом в сообщении пользователя
                        if 'Player is not whitelisted' in com1:
                            sendmessage(id, f'Не удалось удалить {message.split(" ")[2]} из вайтлиста. Был ли он в нём вообще?')
                        else: sendmessage(id, f'Удалил {message.split(" ")[2]} из вайтлиста') #сообщаем что всё хорошо
                    else:
                        sendmessage(id, 'Нужно написать команду вида "Вайтлист {добавить/удалить} {никнейм}"') #если написали "вайтлист" но другие слова неправильные, то выведт это сообщение
                else:
               ...
               banplayer(message,id) #выполняем функцЫю, она сверху
                ...
                if id in adminsid: # если айди в списке айди админом
                    ...
                    if 'Ошибка' in com1: # если в выводе команды ошибка, то пишем это написавшему
                        sendmessage(id, f'Произошла ошибка при разбане игрока {nick}. Игрок существует? Команда написана '
                                        f'правильно?')
                    else:
                ...
                if id in adminsid:
                    if len(message.split(" ")) == 3: # если 3 слова введено - кикает игрока с причиной
                        ...
                        if 'Invalid name or UUID' in com1: # проверяет есть ли ошибка в нике
                            ...
                            sendmessage(id, f'Кикнул {message.split(" ")[1]} по причине {message.split(" ")[2]}, вывод {com1}')
                ...
                if id in adminsid:
                    if len(message.split(" ")) == 5:
                        ...
                        sendmessage(id, f'Телепортировал {player} на координаты x{x} y{y} z{z}') #ну а здесь он тпает исходя из переменных
                    else: sendmessage(id, 'Команда телепорта введена неправильно. Нужно: tp НИК X Y Z')
                else: sendmessage(id, 'Ты не админ!')

            elif message.split(" ")[0] == 'всем:':
                if id in adminsid:
                    com1 = rcon.command(f'broadcast {message.replace("всем:","")}') #просто шлёт всем сообщение через команду /broadcast, она в essentials
                    sendmessage(id,f'Написал на сервер собщение: {message.replace("всем:","")}') #можно заменить /tellraw @a "{message.replace("всем:","")}", одна хуйня получится
                else: sendmessage(id, 'Ты не админ!')

            elif message.split(" ")[0] in donate: #выдать донат посредством LuckPerms
                if id in adminsid:
                    com1 = rcon.command(f'lp user {message.split(" ")[1]} parent addtemp {message.split(" ")[2]} {message.split(" ")[3]}')
