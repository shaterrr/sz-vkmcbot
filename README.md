...
Думаю поймёшь, там всё закоменчено шо пиздец, только дебил не поймёт. Чтобы бот заработал тебе нада запустить vkbot.py, это делается коммандой ```python vkbot.py``` в терминале (хоть линукс хоть виндовс, главное от имени администратора)    
Ну и всё, если не еблан то должно заработать. Если бот написал в вк "Бот успешно запущен", то значит бот запущен. А если написал **"Ошибка  подключения к серверу по причине блаблабла"**, то он попытается ещё три раза подключиться, но если не получится то значит ты чота налажал, это уже не мои проблемы XDD (ну чиркани там https://vk.com/szarkan, обьясню чо куда)  
Ну и если вк апи упадёт(а он упадёт, это не зависит от меня), то бот сам себя перезагрузит. Если же какая другая ошибка, то ботик вам об этом скажет, по крайней мере должен XD ;-))) 🤯  
...
Все логи будут в папке с самим кодом под названием ```log.txt```, 20 знаков вопроса обозначают запуск бота, там кароче он всё логирирует, даже если игрок попытался ввести админскую команду   
